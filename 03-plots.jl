...
# ╔═╡ c745ca89-3d82-4f25-80dd-70c070660eab
begin
...
# ╔═╡ e4343c96-142d-44d4-adb4-bacfdde6e28c
html"""
<p style="font-size: 50px; text-align: center;">实验三 数据可视化  </p>
"""
...
md"""
## 输入数据
Plots.jl包绘图，默认输出的是**序列**。 一个序列就是由多个点和点之间的连线构成。因而， 绘制一条序列， 需要给定这个序列包含的点的坐标。 对二维图形，一个点是由两个坐标（x,y）构成， 一个由多个点构成的序列，则必须要给定这多个点对应的坐标。

一般情况下，  
- 给定一个变量调用， plot(y): 会将这个变量解释为点的y值， 然后自动生成x值去绘图, x值默认就是y值的索引。比如，给定一个向量去绘图， 得到的是，向量值视为y值，对应的下标视为x值的序列的图形。
- 同时给定多个数据源时（多个变量）， plot(x, y)画二维图形， plot(x, y, z) 画3维图形。

需要注意的是： 如果输入的数据是 $n \times k$ **矩阵**， 默认情况下则是画 k 个序列， 也就是每一列当成一个序列去画图。 这个原则在给图形的属性赋值的时候也成立， 即向量形式的属性会作用到每一个序列的相应的点上去， 而矩阵形式的属性， 每一列分别作用到不同的序列上去。 仔细对比下面两幅图形就明白了。
```julia
...
# ╔═╡ d2e0ad90-7822-41fb-8ad2-2032068fc117
plot(rand(10))
...
plot(rand(10,4), markershape = [:circle :rect :star4 :diamond ])
...
- 在绘图的[序列方面](https://docs.juliaplots.org/stable/generated/attributes_series/)： 可能需要设置：
  ...
  - markershape： 点的形状，可选值包括 [:none, :auto, :circle, :rect, :star5, :diamond, :hexagon, :cross, :xcross, :utriangle, :dtriangle, :rtriangle, :ltriangle, :pentagon, :heptagon, :octagon, :star4, :star6, :star7, :star8, :vline, :hline, :+, :x].
...
manifest_format = "2.0"
project_hash = "e7d474c255806fe590058b2839faa8b38f770cbe"
